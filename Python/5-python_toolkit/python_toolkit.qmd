...
Before we begin, please follow the setup instructions.

1. Open your preferred IDE (e.g. Spyder).
2. Download files
3. Change the working direcotry?
...
folder_check = os.path.exists("texts")
print(folder_check)
```
...
1. Create a *set* of unique words with `set(macbeth_words)`
...
Finally, if you want to remove the trailing `.txt` on the messages, you can *slice* the string with square brackets. Specifically, the slice `text_path[:-4]` means slice from the start until the fourth-last character.
...
All together, with the code from Part 1, we can remove the duplicate `files_in_texts = os.listdir("texts")`, so all together: 
...
Let's make a new function now, `read_book_words(...)`, which reads a text file and returns a list of its words, like we do in the loop
...
for text_path in files_in_texts:
    words = read_book_words(f"texts/{text_path}")   # <-- Custom function call
