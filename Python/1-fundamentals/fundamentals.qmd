...
These materials still refer to Spyder, and are subject to change over the next few months. The main sections affected are

* [Setup](#setup)
* [Introducing Python and Spyder](#introducing-python-and-spyder)
* [Scripts and projects](#scripts-and-projects)
* [Installing external packages](#installing-external-packages)

The Python content will remain unchanged.
...
If this is too easy, try to get the user to provide their age themselves with the command `int(input(...))`. **Make sure to use `int()`**, otherwise the string multiplication will crash Spyder. You'll want the [documentation for `input()`](https://docs.python.org/3/library/functions.html#input).
:::
...
1. [Download the data](BMI_data.csv)
