...
{{< embed git_banner.qmd >}}

## Installation

We will use Git inside a command-line shell called Bash.

* If you are using a UQ Library computer: open ZENworks and install "Git for Windows". Once installed, launch "Git Bash" from the start menu.
* If you are using a UQ computer: open Company Portal and install "Ubuntu 24.04" to access Git through the Windows Subsystem for Linux (WSL). Once installed, open "Ubuntu" from the start menu.
* If you have your own computer, follow the instructions for your operating system on the Git website: [Linux](https://git-scm.com/install/linux), [macOS](https://git-scm.com/install/mac), [Windows](https://git-scm.com/install/windows)
...
![Committing changes to a single file (CC BY 4.0, [HSF Analysis Essentials](https://hsf-training.github.io/analysis-essentials) contributors)](https://hsf-training.github.io/analysis-essentials/_images/git-staging-area.svg){fig-alt="Illustration of the process of adding a file change to a staging area, and then committing it to the git history, represented as a stack of cylinders with the most recent at the top."}
...
![Committing changes to several files, as a single commit (CC BY 4.0, [HSF Analysis Essentials](https://hsf-training.github.io/analysis-essentials) contributors)](https://hsf-training.github.io/analysis-essentials/_images/git-committing.svg){fig-alt="Illustration of the process of adding changes to several files to the staging area, and then committing it to the git history, represented as a stack of cylinders with the most recent at the top."}
...
![Checking out files in their state at a specific commit (CC BY 4.0, [HSF Analysis Essentials](https://hsf-training.github.io/analysis-essentials) contributors)](https://hsf-training.github.io/analysis-essentials/_images/git-checkout.svg){fig-alt="Illustration of checking out files at a specific commit: the symbolic stack of commits is split open to show the state of the files one commit before HEAD, which is what is restored by the command 'git checkout' follow by a commit identifier."}
