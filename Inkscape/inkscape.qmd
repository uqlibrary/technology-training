...
This session was designed using Inkscape 1.4.3.
...
Once the vector version of the image is cleaned up and brightened, place it in the bottom-left corner of the page. If needed, you can flip objects by using the menu: Object - Flip Horizontal/Vertical.

## Layers and Objects

In the Dialogue panel, the "Layers and Objects" dialogue is a very important tool, especially as your composition becomes more complex. It shows in which order all the layers are stacked, and also makes it more obvious that you are inside a group of objects (like our traced flower). You can drag-and-drop objects to change the order they are stacked in, and choose to hide and lock them (so you don't select or modify them by mistake).
...
You can now lock so you don't move or select it by mistake.
...
Use it to trace a path of straight segments, clicking to place each node, and right-click when you are finished. You can also double-click when placing the last node.
...
::: {.callout-note}
When modifying your arrows, pay attention to the difference between using the Selector Tool and the Node Tool:

* The arrows around a selected item will stretch and squish your object, changing the thickness of the line, making the arrows inconsistent.
* On the other hand, editing the object with the Node Tool with allow you to change the length of the path, its curvature, and the direction of the arrow without changing the general appearance.
:::
