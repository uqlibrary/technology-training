...
{{< embed dplyr_banner.qmd >}}

## What are we going to learn?

In this hands-on session, you will use R, Positron and the `dplyr` package to transform your data.
...
3.  the keyboard shortcut: press F1 after placing your cursor on a function name
4.  the Help pane: use the Search box in the Help pane to find functions, packages, topics
...
-   On Library computers (the first time takes about 20 min.):
    -   Log in with your UQ credentials (student account if you have two)
    -   Make sure you have a working internet connection
    -   Open the Start menu
    -   Search for and open ZENworks
    -   Look for R - Statistical Computing and double-click it to install
    -   Look for Positron and double-click it to install
    -   Open Positron through the Start Menu once the installation is complete
...
::: {.callout-note}
At home, you can install the whole "[tidyverse](https://www.tidyverse.org/)", a meta-package useful for data science, which includes dplyr: `install.packages("tidyverse")`
:::
...
::: {.callout-note}
 Remember you can use <kbd>Ctrl</kbd> + <kbd>Enter</kbd> (or <kbd>Cmd</kbd> + <kbd>Enter</kbd> on a Mac) to execute a command from the script.
:::
...
The [dplyr website](https://r4ds.hadley.nz/) introduces the package as follows:
...
```{r}
#| label: filter-out-europe
no_europe <- filter_out(gapminder, continent == "Europe")
# getting the unique values confirms it has worked:
unique(no_europe$continent)
```

::: {.callout-warning}
## Handling of missing values

Both `filter()` and `filter_out()` interpret `NA`s as ``FALSE`. This means that `filter()` discards the rows that return `NA`, and `filter_out()` keeps them, making the two functions complimentary.
...
```{r}
#| label: na-handling
nrow(filter(starwars, hair_color != "blond")) # discards NAs
nrow(filter_out(starwars, hair_color == "blond")) # keeps NAs
```

If the intent is to "drop starwars characters with blond hair", the second command is preferred because it will keep the characters that don't have data about hair colour.

Wrapping up the topic of missing data, here are other useful functions:

* For creating conditions based on NA values, use the `is.na()` function.
* For handling missing data more easily, see tidyr's [`drop_na()`](https://tidyr.tidyverse.org/reference/drop_na.html) and [`replace_na()`](https://tidyr.tidyverse.org/reference/replace_na.html) functions.
:::
...
* `slice()`: by row index
* `slice_sample()`: random sample of rows (number or fraction)
* `slice_max()` and `slice_min()`: rows that hold a variable's highest or lowest values
* `slice_head()` and `slice_tail()`: beginning or end rows
...
#| label: slice-max
#| eval: false
slice_max(gapminder, lifeExp, n = 10)
```
:::
...
... but this becomes very hard to read (and write), and gets worse with more steps!
...
Here's another example with the `filter_out()` verb:

```{r}
#| label: no-france-pipe
#| eval: false
gapminder %>%
  filter_out(country == "France")
```
...
#| eval: false
filter_out(gapminder, country == "France")
```
...
Writing multi-step processes with the pipe has many benefits:
...
The data in the cells is the same, the size of the object is the same. However, the dataframe was converted to a **tibble**: an augmented version of the dataframe. That's because a dataframe is not capable of storing grouping information.
...
-   *R for Data Science*, [chapter about dplyr](https://r4ds.hadley.nz/data-transform)
