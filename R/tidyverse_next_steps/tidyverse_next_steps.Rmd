...
To go from wide format to long format, we can use the tidyr function `gather()`. Here, we want to gather all the columns titled with a year: we store the data in a "value" variable, and the year in a "year" variable (the key).
...
We set `convert` to TRUE because we want the function to change the key class from character to numeric.
