...
What if we prefer to use horizontal bars? We only need to switching the mapping of the variable to the `y` aesthetic:
...
The `theme()` function allows us to really get into the details of our plot's look, but some `theme_*()` functions make it easy to quickly apply a built-in theme, like `theme_bw()`:
