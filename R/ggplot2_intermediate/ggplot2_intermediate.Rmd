...
Notice the warning message? ggplot2 informs us that it couldn't display the data related to China and India. Instead of using `ylim()`, we could use `subset()` (or `dplyr::filter()`) to remove the two countries from the data before feeding it to ggplot2.

The method we use works for our point geometry, but would be problematic for other geometries that would get cut off when zooming in. A better way to focus on one part of the plot would be to modify the **coordinate system**:
...
The cartesion coordinate system is the default one in ggplot2. You could change the coordinate system to `coord_polar()` for circular visualisations, or to `coord_map()` to visualise spatial data.
...
* More resources for R in general: https://gitlab.com/stragu/DSH/blob/master/R/usefullinks.md