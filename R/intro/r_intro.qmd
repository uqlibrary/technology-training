---
title: "R with Positron: getting started"
editor: source
date: last-modified
author: UQ Library
aliases: 
  - "/R/rstudio_intro/rstudio_intro.html"
---

{{< embed r_intro_banner.qmd >}}
...
[Positron](https://positron.posit.co/) is an open source Integrated Development Environment (IDE) for data science, which means it provides many features on top of R to make it easier to write and run code. [RStudio](https://posit.co/products/open-source/rstudio) is another IDE suited for writing R which used to be the preferred tool for this training series.
...
For this course, you need to have both R and Positron installed ([installation instructions](../installation.qmd)).

## Open Positron

* If you are using your own laptop please open Positron
...
* On Library computers:
  ...
  * Search for "Positron"
  * Double-click on Positron which will install both R and Positron
...
* Create and organise a project for data analysis
* Create a script
...
* Understand the different Positron panels
* Use a few shortcuts
...
* Click the "Folder Selector" menu (top right corner), then "New Folder from Template..."
* Click "R Project" and "Next"
* In "Folder name", type the name of your project, for example "YYYY-MM-DD_R-intro"
...
> R Projects make your work with R more straight forward, as they allow you to segregate your different projects in separate folders. You can create a project in a new directory, or open an existing directory that already has R code and data. Everything then happens by default in this directory. You can then jump from project to project from the "Folder Selector" menu.

## Maths and objects

The **console** (usually at the bottom left in Positron) is where most of the action happens. In the console, we can use R interactively. We write a **command** and then **execute** it by pressing <kbd>Enter</kbd>.
...
You should now see your objects listed in you **Variables pane** (top of the right sidebar).
...
For that, we can use the function `dir.create()`:
...
When creating a new project, Positron opens a new script in the **Editor** above the console. You can straight away save this script and name it something like "process.R".

### Many ways to do one thing

As in many programs, there are many ways to achieve one thing.

For example, we used commands to create folders, but we could have done the same in the Explorer panel by either:

* using the toolbar that pops up when hovering the project folder name, or
* using the menu that pops up when right-clicking the empty space

Learning how to use functions rather than the graphical interface will allow you to integrate them in scripts, and will sometimes help you do things faster.
...
> While editing your script, you can run the current command (or the selected block of code) by using <kbd>Ctrl</kbd>+<kbd>Enter</kbd>. Remember to save your script regularly with the shortcut <kbd>Ctrl</kbd>+<kbd>S</kbd>. You can find more shortcuts with <kbd>Ctrl</kbd>+<kbd>K</kbd> followed by <kbd>Ctrl</kbd>+<kbd>S</kbd>, or the menu: "Manage (cog icon) > Keyboard Shortcuts".
...
There are several ways to find **help** about a specific function in Positron:

1. the command `help(functionname)` or its shortcut command: `?functionname`
...
1. hover over a function name with the pointer to see a popup
...
> All the file paths are **relative** to our current working directory: the project directory. To use an **absolute** file path (i.e. one that starts from the top of your drive), we can start with `/`.
...
> Positron's Variables pane already shows us some of that information (click on the arrow next to the object name).

And to explore the data in a viewer, click on the table icon next to the object in the Variables pane.

This viewer allows you to explore your data by scrolling through, searching terms, filtering rows and sorting the data. It also has a handy "Summary" panel that you can expand to see descriptive statistics and histograms. Remember that it is only a viewer: it will never modify your original object.

> You can also use the `View()` function to open the Viewer. Feel free to use that instead of clicking on the button, but note that the case matters: using a lowercase "v" will yield an error.
...
> Remeber to use the <kbd>Tab</kbd> key to autocomplete your function names, object names and file paths! It saves time and makes typos less likely.
...
Packages add functionalities to R and Positron. There are [more than 24000](https://cran.r-project.org/web/packages/index.html) available.
...
1. Search for "skimr" and select it
...
You can close Positron after making sure that you saved your script.

When you reopen a project in Positron, it brings back whatever source files (e.g. scripts) you had open, and your command history. You will find your command history in the "History" tab (right sidebar): all the commands that we used should be in there. The console, on the other hand, only shows a brand new R **session** when you reopen Positron. Sessions are not persistent, and a clean one is started when you open your project again, which is why you have to load any extra package your work requires again with the `library()` function. The best way to work is to run your script again every time you go back to your project.

## What next?

* We have a [compilation of resources](../usefullinks.qmd) for the rest of your R learning
