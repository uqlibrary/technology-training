...
Within QGIS, go to the **Browser** panel, open `**Project Home** > **data** > **raw**`, then drag and drop each of your files into the **map canvas**.
...
The random colours that QGIS gives you aren't always pretty, so let's change the colours - Click the dropdown next to the `Color ramp` - Click `Create New Color Ramp...`, select `Catalog: ColorBrewer`, and click `OK` - Click the dropdown next to `Scheme name` and choose `**Set1**` - This is a qualitative colour scheme that is easy to distinguish, **Dark 2** and **Paired** are also good options, you can find out more about [ColorBrewer here](https://colorbrewer2.org) - Click the `back arrow button` next to **Edit Ramp**
...
-   Click the dropdown next to `Stroke color`, click `Pick Color`, and then click the color from the `Fill Color` above, it should copy the colour!
...
Repeat these steps for the **Single Artefact**, you can leave the shape as a circle.

If you want, you can change the properties of all symbols at once: - Simply click in the white space below your symbol classes so that non is selected. - If you click on the button next to `Symbol`, it will take you to `Symbol Settings` - Try increasing the size of the symbols to 3mm - **WARNING** if you change the **Stroke color** here, it will change the stroke for all of your symbols, undoing your work from above. - Click the `back arrow button` next to **Symbol Settings**
...
-   Click the **green +** next to `Line`

You can add as many simple lines, as you like, you can even change this `Symbol layer type` to Arrows, Hashed lines, Filled lines, and more.
...
-   In the `Layers` panel, hide the **Paths** and **Artefacts** layer by unticking the boxes next to them.
...
-   Unhide **Paths** and **Artefacts** layer by ticking the boxes next to them.
-   Change the **Project_area** back to our original **Default** style
    -   `Right click on **Project_area** > Styles (2) > default`

Nothing exciting will happen *yet*, we will be able to use this later.
...
-   Right click WMS/WMTS and select `**New Connection...**`
    ...
    -   In `URL` paste https://services.arcgisonline.com/arcgis/rest/services/World_Imagery/MapServer/WMTS/1.0.0/WMTSCapabilities.xml
    -   Click `OK`
-   Now in the `Browser` panel, navigate `WMS/WMTS > ESRI World Imagery` and double click on **default028mm**
    -   The **GoogleMapsCompatible** is the same map, but a lower resolution, however **default028mm** will often show "Map data not yet available" at a scale around **1:300**
...
-   Now, click the **Eye** icon again
    ...
    -   If you want to update a theme, simply go `**Eye** icon > Replace theme...` and then select the theme you want to overwrite.
-   Save your project
...
-   In the `Items` panel on the right, double click on **Map 1** and rename it **Main**
    -   This makes life so much more straightforward

### Scale

Okay, we have our map, but it's not zoomed in right. This is always a little tricky. You *could* click the **Move item content** button on the left menu(or press `C` on your keyboard), as this would allow you to drag the map around internally, and zoom in with your scroll wheel. **But there is an easier way**
...
We don't need more guides unless you want a precise inset map size, but I'm happy if it's aligned evenly with the top and side border.
...
-   Scroll to the bottom of the `Item Properties` panel and tick the `Frame` box
    -   This gives our inset a nice border we can customise
...
-   Now repeat those steps for the rest, click next to **Scale: 1:**
    -   Click `Dynamic Text > Map Properties > Main > Scale (4000)`
    -   The code will look like this: `[%format_number(item_variables('Main')['map_scale']` which automatically adds the main map's scale - Click next to **Datum:**
    -   Click `Dynamic Text > Map Properties > Main > CRS Name (GDA2020 / MGA zone 56)`
    -   The code will look like this: `[%item_variables('Main')['map_crs_description']%]` which automatically adds the main map's CRS - Click next to **Sources:**
...
You could also add any extra text details such as your name, the locality, any disclaimers, or any other details needed.
...
-   Add a scale bar to your map by clicking the **Add Scale Bar** icon in the left menu, or navigating `Add Item > Add Scale Bar`
    ...
    -   In the `Item Properties` panel, choose the appropriate `Scalebar units` - usually **Metres** or **Kilometres**
        -   Under **Segments**, change right `Segments` to **right 4**
