...
First we will create a field to represent the original area of our KPAs.

-   Select **KPA_Reproj** from the `Layers` panel.
...
-   Click on the **Field Calculator** button ![image](https://user-images.githubusercontent.com/67612228/195319687-1538f2a0-d8c8-4084-a632-0e3181fb11dd.png){alt="image"}
    ...
    -   Under **Create a new field** set the **Output field name** to `KPA_Orig_Area`
    ...
    -   In the `Expression` tab enter `$area`

    -   Click OK

-   Click `Save Layer Edits` ![image](https://docs.qgis.org/3.44/en/_images/mActionSaveEdits.png){alt="Save Layer Edits Button"}

-   Click `Toggle Editing` ![image](docs.qgis.org/3.44/en/_images/mActionToggleEditing.pngg){alt="Toggle Editing Button"}
...
If we want to we can now `Dissolve` the layer based on the **fid_2** field to recombine the original areas back together to make calculating the area easier.
